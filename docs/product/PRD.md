...
| Status | Draft v5.0 — vertical slice with a walkable pixel-art overworld hub (scope change, §7.1), in-engine art, full ability kits, real per-visit encounter variety on every non-boss node, fully-documented internal schemas, and a verified-green (Chromium) CI/deploy pipeline; remaining gaps are either external-resource-blocked (art, real audio, real-device QA, Firefox root-cause) or pure follow-on content work — pending stakeholder sign-off |
| Owner | Amir Bukhari |
...
1. **Art is placeholder, not a final art-bible pass.** The single "Amir" placeholder sprite (§11.4) now renders in-engine in battle *and* walks the overworld (§20.1), but it is reused/tinted across all four heroes rather than being distinct per-class art, the overworld walk animation is one run strip covering all four facings, and the overworld tiles are programmatically-generated flat-color placeholders; there is still no `docs/design/art-bible.md` and no real spritesheet/animation work. Blocked on real art asset production, not an engineering task this environment can close by itself.
