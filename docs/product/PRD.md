...
| Status | Draft v3.0 — vertical-slice exit condition met (one full biome, one boss); pending stakeholder sign-off |
| Owner | Amir Bukhari |
...
This section is a factual snapshot of what exists in the repository against this PRD, kept separate from the spec itself so the spec stays a stable target while this section is expected to go stale and get re-cut periodically. Where this section and earlier sections disagree on what's "done," this section is authoritative for current reality; the numbered sections above remain authoritative for what's *required*. This is the second cut of this section (v1 is preserved in the revision history, §0) — most of what it listed as open is now closed; what's genuinely still open is in §20.2 below.

### 20.1 Built, tested, and verified live

Everything below was checked with `npm run typecheck`, `npm test` (94 unit tests passing), `npm run build`, and `npm run test:e2e` (11 Playwright specs), plus extensive manual live verification in headless Chrome/Firefox (scripted keyboard/pointer input, screenshots, console/error inspection, and direct state manipulation via a dev-only debug hook for scenarios real-time input automation can't drive precisely, like boss HP thresholds) — not just typechecked in isolation.
...
| **Relics and skill unlocks** | `ResultsScene` presents real relic choices; chosen relics apply real mechanical effects at battle start (`Relics.ts`: +1 max focus / permanent tank guard / banked groove). Defeating a boss writes real tier-2 unlock ids to `SaveProfile.unlockedSkills` — the trigger and persistence are real, though no tier-2 *ability content* exists yet to attach to them (deliberately not invented unscoped; see §20.2). |
...
| **`tests/e2e/`** | 11 committed Playwright specs (Chromium + Firefox config) replacing ad hoc scripts: boot/calibration/reload persistence, battle mechanics, the boss's 3-phase mechanic, and two settings-UI regressions. Wired into CI. See caveat in §20.2. |
...
1. **No visual art in-engine.** Every scene is still text/shape rendering only. The placeholder "Amir" sprites (§11.4) are not loaded or drawn anywhere; the pixel-art pipeline is configured but unexercised by real sprite content.
2. **Music is placeholder sonification, not the real soundtrack.** `BeatmapSonifier`'s synth blips are a scratch layer for feeling the beat. The seven `tools/gbmusic/` chiptune drafts are not wired into the game — they'd need to be rendered from `.lsdsng` to playable audio first.
3. **Tier-2 ability content doesn't exist.** The skill-unlock *mechanism* is real (§20.1), but the PRD's v1 kit table (§8.4) only specifies 3 abilities per role; authoring a 4th per role is unscoped content work, not a code fix.
4. **E2E suite has a documented sandbox-specific flakiness caveat.** In this development sandbox specifically, sustained sequential headless-WebGL sessions cause occasional browser crashes unrelated to game logic (isolated via extensive testing: worker count, boot redundancy, dev vs. production build, raw Playwright API vs. test runner — see `tests/e2e/README.md`). Chromium is reliable serially; Firefox is flakier in this sandbox. CI (a real, non-sandboxed runner) is expected to be more stable and is now the authoritative signal, since it's wired into `deploy-pages.yml` — check its results before assuming this caveat still applies.
5. **QA matrix (§16) unexecuted on real browsers/devices.** Verification has been headless Chromium/Firefox via automation, not a real Chrome/Edge/Firefox/Safari desktop pass or any device/hardware testing.
6. **Content is one encounter deep per stage.** Each of the 5 campaign nodes has exactly one authored encounter. Re-playable variety (multiple encounters per node, branching paths) doesn't exist — the campaign is a single linear chain.
7. **Map UI is minimal.** `MapScene` is a text list with lock/cleared status, not the visual node-graph-on-a-path the PRD's "node-based campaign map" implies.
8. **`Enemy`, `HeroClass`, `CampaignNode`, and `BossPhaseConfig` are undocumented-by-PRD internal schemas.** A deliberate, documented scope decision (see rationale comments in `src/data/schemas/`), not scope creep — the PRD's three canonical schemas intentionally don't cover combat stats, map topology, or boss phasing, and something had to for the game to be runnable at all.

### 20.3 Suggested next increment

The vertical-slice exit condition is now met. The next bounded increments, roughly in order of value: (1) resolve the e2e sandbox-flakiness caveat by checking actual CI results and tightening `tests/e2e/` accordingly; (2) art integration -- replace text rendering with real sprites, gated on the still-unwritten `docs/design/art-bible.md`; (3) render the `tools/gbmusic/` chiptune drafts to real playable audio and wire them in, replacing `BeatmapSonifier`; (4) author a second full biome (more encounters per node, second boss) to move from "one biome, one boss" to the fuller campaign shape in §8.6.
