...
| Status | Draft v4.0 — vertical slice with in-engine art, full ability kits, and a visual campaign map; pending stakeholder sign-off |
| Owner | Amir Bukhari |
...
3. **E2E suite has a documented sandbox-specific flakiness caveat.** In this development sandbox specifically, sustained sequential headless-WebGL sessions cause occasional browser crashes unrelated to game logic (isolated via extensive testing: worker count, boot redundancy, dev vs. production build, raw Playwright API vs. test runner — see `tests/e2e/README.md`). Chromium is reliable serially; Firefox is flakier in this sandbox. Re-confirmed this cycle: an ad hoc full-suite run hit the same "target page/context/browser closed" crash signature on `settings.spec.ts`, unrelated to any code changed in this cycle (the affected spec doesn't touch the files that changed). CI (a real, non-sandboxed runner) is expected to be more stable and is the authoritative signal, since it's wired into `deploy-pages.yml` — check its results before assuming this caveat still applies.
