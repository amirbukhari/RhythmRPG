...
| Status | Draft v4.5 — vertical slice with in-engine art, full ability kits, a visual campaign map, real per-node encounter variety (1 of 5 nodes), fully-documented internal schemas, a verified-green (Chromium) CI/deploy pipeline, and — new this revision — real chiptune battle music playing in-game: all 7 stage/boss-phase tracks rendered from the committed `.lsdsng` drafts by a purpose-built Game Boy APU synthesizer (`tools/gbmusic/render_lsdsng.py`), closing the audio-rendering blocker previously thought to require PyBoy/LSDJ. Remaining gaps are either external-resource-blocked (art, hand-tuned/DAW-quality audio, real-device QA, Firefox root-cause) or pure follow-on content work — pending stakeholder sign-off |
| Owner | Amir Bukhari |
...
| 4.5 | 2026-07-10 | Amir Bukhari | Closed §20.2's music-rendering blocker without PyBoy or the LSDJ ROM: since the 7 committed `.lsdsng` drafts were generated by this repo's own `lib/midi_to_lsdsng.py` (known structure: note starts on a 16th grid, one instrument per channel, no FX/tables), a direct Game Boy APU synthesizer over the parsed note data is sufficient — built `tools/gbmusic/render_lsdsng.py` (pulse ×2 / 4-bit triangle wave / 15-bit LFSR noise, GB-stepped envelopes) and `render_all_tracks.py`, which renders every track at its beatmap's authored BPM cut to an exact whole multiple of the beatmap's pattern loop (transcribed tempi and beatmap BPMs match 1:1). All 7 tracks now play in-battle via `ChiptuneMusicPlayer`/`BattleTracks` (preloaded per-fight, bar-aligned looping, boss phase swaps on the bar boundary, sonifier retained as the downbeat/telegraph cue layer). Also vendored `lib/bread` (pylsdj's parser dep; unbuildable sdist on current setuptools). Fixed a real latent scene-reuse bug this work exposed: `BattleScene.stage` was never reset in `create()`, so during the new async audio preload a relaunched battle reported a stale `"command"` stage before its input handler existed — input during that window was silently eaten (same Phaser persistent-Scene-instance class of bug as v4.2's `SettingsOverlay` fix); added a `"loading"` stage. 105 unit tests (was 102); e2e suite now 13 Chromium tests (was 11), including music-actually-playing assertions in battle and across a boss phase swap. |
...
**Rendering decision (v4.5):** the drafts no longer require LSDJ (or any Game Boy emulator) to become audio. `tools/gbmusic/render_lsdsng.py` synthesizes the four APU channels directly from the parsed `.lsdsng` note data, and `render_all_tracks.py` produces the in-game tracks (`assets/audio/battle/*.ogg`), each rendered at its beatmap's authored BPM and cut to an exact whole multiple of the beatmap's pattern loop so it loops bar-aligned in-battle. This is the *placeholder-shippable* path — it faithfully renders what the generator writes (it ignores LSDJ FX/tables the generator never uses), so a future hand-tuning pass in LSDJ either re-renders through this same tool (if it sticks to notes) or graduates to a real LSDJ render per the emulator/hardware path in `tools/gbmusic/README.md`. The DAW-authored-stems spec above remains the bar for final audio delivery.
...
| **Real chiptune battle music (§11.2)** | All 7 stage/boss-phase tracks render from the committed `.lsdsng` drafts to Game Boy-style audio (`tools/gbmusic/render_lsdsng.py` — direct APU synthesis, no LSDJ ROM/emulator needed) and play in-battle via `ChiptuneMusicPlayer`: preloaded per-fight, looped bar-aligned with the judgment grid (each file is an exact whole multiple of its beatmap's pattern loop at the authored BPM), boss phase transitions swap tracks on the bar boundary, game-speed setting stretches playback identically to the transport. Machine-transcribed placeholder *quality* (§20.2), but real, playing music — no longer a silent scheduler wearing a synth-blip click track. |
| Audible battle timing | `BeatmapSonifier` schedules real, meter-aware Tone.js synth hits for a beatmap's events, looped and phase-aware — retained as the downbeat/telegraph cue layer over the music above, since a machine-transcribed track doesn't guarantee an audibly marked downbeat by itself (§7.3 rhythm clarity). |
...
2. **Music is machine-transcribed placeholder quality, not hand-tuned final stems.** The former blocker here — no way to render `.lsdsng` to audio at all without PyBoy (no export API) or the LSDJ ROM (not committable) — is closed as of v4.5: `tools/gbmusic/render_lsdsng.py` synthesizes the APU channels directly and all 7 tracks now play in-game (§20.1). What remains is *quality*, not plumbing: the note data underneath is still an untuned machine transcription (basic-pitch misfires, single-pitch onset-triggered drums, flattened polyphony — see `tools/gbmusic/README.md` limitations), no human has auditioned the slice-to-stage mapping (`docs/design/music-direction.md`), and the §11.2 delivery spec (stems, tempo/meter maps, SFX pack, hand-tuning pass) is unmet. Blocked on a human listening/hand-tuning pass — musical judgment, not engineering.
...
Three things previously listed here are now closed and no longer gaps: (a) the four internal schemas (`Enemy`, `HeroClass`, `CampaignNode`, `BossPhaseConfig`) are now documented in §10.5 alongside the three canonical ones; (b) two Chromium `settings.spec.ts` failures previously chalked up to "environment flakiness" were root-caused as real bugs (`TextMenu` double-firing a single keypress via Phaser's keyboard-queue reprocessing, and `SettingsOverlay` reuse-after-stop referencing destroyed GameObjects and soft-locking the player) and fixed — see `src/ui/components/TextMenu.ts`, `src/scenes/SettingsOverlay.ts`, and the v4.2 revision-history entry; (c) the `.lsdsng`→audio rendering blocker is closed and all 7 tracks play in-game (v4.5 — item 2 above now tracks only the remaining *quality* gap). The full Chromium e2e suite and the GitHub Pages deploy pipeline are both verified green as of this revision.

### 20.3 Suggested next increment

The vertical-slice exit condition is met and the previous cycles' engine-facing gaps (art rendering, tier-2 content, map UI, schema docs, CI/deploy health, and now audio rendering/playback) are closed. What's left splits into two kinds:

**Blocked on something outside this environment** (not closeable by more engineering effort alone): (1) a human listening/hand-tuning pass on the now-audible tracks — audition the slice-to-stage mapping (`docs/design/music-direction.md`), hand-tune the `.lsdsng` drafts in LSDJ, re-render via `tools/gbmusic/render_all_tracks.py`, and eventually meet the full §11.2 stem/delivery spec; this is musical judgment and LSDJ authoring work, not engineering; (2) a real art-bible pass and distinct per-class spritesheets, gated on `docs/design/art-bible.md` and actual art production; (3) the QA matrix (§16) on real, non-headless browsers/devices; (4) the Firefox e2e root-cause (needs a maintainer with a non-sandboxed environment to bisect against, since both this sandbox and real CI show the identical failure).
