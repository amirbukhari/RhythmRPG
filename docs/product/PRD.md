...
| **9.7.1** | **2026-07-15** | *(unreleased — deploys frozen until the owner's HLD bar is met)* **Every prop is placed with intention.** Owner: \"why is there a random bottle scattered for no reason. items need to be placed with intention. none of this scattering shit... why do I see 70 bottles in one scene.\" Root cause: the hash-scatter stamped the same few kit pieces across hundreds of cluster tiles while most of the library never appeared. The scatter system is **deleted** (hash loop + hash landform placement). Replacement: **authored set-dressing** (`src/data/content/overworld/dressing.json`) — 215 placements composing **88 story vignettes** (a tide-watcher's cold camp, the wreck of the Pale Sister, a figurehead shrine, a foreman's abandoned shift…), singletons, and landmark landforms, authored by a 10-agent workflow (one scene-dresser + one adversarial placement-verifier per region) against the region terrain maps, then hard-validated by `tools/overworld/merge_dressing.py` (anchor-on-grass, venue/echo clearance, per-key budgets: scatter ≤2, filler ≤4, landform ≤1 per region). Each region reads sparse and deliberate; no piece repeats more than twice per region. 115 unit / 17 e2e green. |
