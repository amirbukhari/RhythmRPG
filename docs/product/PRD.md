...
| Version | Date | Change |
|---|---|---|
| **14.3** | **2026-07-21** | **Ambience pauses during fights (CI/low-end fix).** The v14.2 pass passed local e2e but the deploy's Chromium gate timed out the fight specs on the weaker CI runner: the whole ambient layer (fauna, particles, NPCs, eelgrass tweens, `driveAmbience`) kept animating *during* in-world fights, starving the fight's frame budget on slow software-WebGL (and peaking memory). Fix: every ambient object is tracked, and on fight-enter the layer is hidden and its tweens paused (`setAmbientHidden`) with `driveAmbience` skipped — the fight owns the frame; ambience restores on exit. Particle/fauna/NPC counts also trimmed for headroom. Full Chromium e2e green in CI. |
| **14.2** | **2026-07-21** | **The world is alive — a full dynamic-ambience pass.** Owner: "do literally all of those." Five layers of motion, all cosmetic (gameplay timing stays on TransportClock, §10.2), deterministic, reduced-motion-gated, fixed object pools (soak-safe): **(1) a beat-reactive sanctuary** — the obelisk's crown-light and the whole kneeling congregation pulse/bow on the *actual* beat of the overworld track (deereater, 103.36 BPM) read through its measured beat grid via `music.position()` + `nearestBeatDistanceSeconds`, with a steady-BPM fallback when audio is blocked; **(2) drifting fauna** — loose fish schools and pulsing jellyfish in the drowned water column, crabs scuttling the seafloor, an eel coiled by the rocks; **(3) region-keyed airborne particles** across all five biomes — pale spray over the Breach, ash + rising embers on the Scar, drifting violet spores on the Stage approach (the drowned regions already breathe motes/eelgrass from v14.1); **(4) roaming NPCs** — pilgrims pacing the roads on validated segments, a lone fisher at the Breach waterline; **(5) weather beats** — a current surge that rolls every eelgrass blade together every ~20s, distant lightning over the Stage (behind photosensitivity safe mode), tide shimmers sweeping the waterline. Driven from a single `driveAmbience(dt)` doing only cheap transforms per frame. typecheck, 115 unit, full Chromium e2e green. |
| **14.1** | **2026-07-21** | **No puddles underwater — and the drowned world moves.** Owner: "there should be no puddles in the underwater area. what else can we add to make the environment more dynamic?" **(1)** The drowned regions (Fold + Kelp Shelf) no longer carry standing water — a discrete puddle on the ocean floor read wrong. The generator places rock and silt there, never water tiles (the Breach waterline and the surface lakes keep theirs), and the plate is repainted seamless seafloor. **(2)** In their place the water column is alive: `addDrownedLife` seeds rising sediment/bubble motes off the seafloor and beds of eelgrass that sway in the current across the drowned regions — deterministic, reduced-motion-gated, a fixed ~110-object tween pool (soak-verified leak-free). Overworld + obelisk + soak e2e green. |
| **14.0** | **2026-07-21** | **The sanctuary — the Fold is a town at prayer, and combat waits outside it.** Owner: "the underwater area needs to be an underwater town where they all pray to a massive obelisk. the campaign and fights etc shouldn't really start til the player exits the underwater zone." Completing the v12.0 premise the ground had only half-told: **(1) a MASSIVE town obelisk** now stands at the plaza heart — a tall two-faced monolith with a carved teal glyph-seam and a pulsing crown-light, ringed by **kneeling worshippers** bowed toward it (procedural, sprite-free per the v11.2 register; motion-gated), drawn by `OverworldScene.placeTownObelisk` off a new `town_obelisk` marker and doubling as the town's save point ("E: pray"). **(2) The Fold is combat-free** — the first fight (`opening_1`) is lifted out of the town onto the Kelp Shelf at the Fold's mouth (tile 18,30), so no foe stands inside the sanctuary and the campaign begins only on exit. **(3) A threshold beat** — Mir's first step out of region 0 fires once ("THE FOLD BEHIND YOU / The chorus begins."), persisted via `SaveProfile.leftFoldAt`. **(4) The plaza** gains a painted stone dais, a long cast shadow, and concentric worn prayer-focus rings under the monolith. BFS reachability green; 115 unit + overworld/obelisk e2e green. |
| **13.2** | **2026-07-17** | **The big variation pass — the map has RELIEF now.** Owner: "that's it? ... I don't think this was enough variation at all." The two things the map was actually missing: **height** and **landform diversity**. **(1) ELEVATION** — a smooth height field (low in the drowned SW, rising toward the NE Stage: the ascent is literally uphill) quantized into 7 terraces; every south-facing step casts a shadow cliff, every north rise catches a lit lip, faint level contours whisper the slope. The whole world rolls now instead of lying flat. **(2) A RIVER** — a winding water channel with stone fords spills out of the drowned west and snakes across the Scar to the Stage lake, with a flowing current sheen; a reef archipelago scatters the Breach shallows. **(3) ~8 new one-off landmarks** — the Great Wreck (a colossal grounded hull on the Breach shore), the Boneyard (a battlefield of ribcages and scattered bone), the Geyser Field (mineral hot-pools ringed with crust), the Mast Forest (dead ships' masts standing like trees in the drowned Shelf), the Giant Footprint (one vast three-toed track), the Salt Flats (a blinding hex-cracked crust plain), and the Spire Ruin (a toppled lighthouse). **(4) material texture** — per-region water types and road makings (from v13.1) plus salt/basalt cracking. BFS green; 115 unit / sweep + overworld + boss e2e green; verified in-game at real zoom. |
| **13.1** | **2026-07-17** | **Map variation — structure, not just tint.** Owner: "still not sure our map has enough variation to it." Four layers land: **(1) macro landforms** in the generator — a curved two-walled CANYON with gap windows runs down the mid-Scar (the touring road punches its own gate through it) and a broken-ringed CRATER with a tar pit at its heart sits in the east; **(2) Scar sub-biomes** — coarse-noise districts split the huge surface into pale ash wastes, the rust flats, and deep-red thorn barrens, seamlessly; **(3) material variation** — water is typed per region (teal drowned pools, foamless black TAR pits on the Scar, the Stage's ink-violet lake) and the one road changes making as it travels (silt lanes under the sea, packed dirt across the Scar, pale worn paving on the Stage approach); **(4) one-off ground vignettes** — places that exist exactly once: the LEVIATHAN (a bleached whale skeleton, spine/ribs/skull, in the south-east), the FALLEN OBELISK (shattered segments and its impact gouge, west Scar — the cult's stone, face-down), the SILENT RING (nine leaning standing stones on the north rise, each shadowed the same way), and the DRIED LAKEBED (a pale cracked pan in the east). BFS green; 115 unit / sweep + overworld + boss e2e green. |
| **13.0** | **2026-07-17** | **The open world — no more left-to-right.** Owner: "the world feels very left to right eh?" → "we start on the left but it's like you ascend to a world that's extensive to the up down and right and the story takes you through a specific path around the map to a specific point for the ending." The map is rebuilt 112×64 with regions as **organic weighted-Voronoi territories**: the drowned Fold (SW, where Mir wakes) and Kelp Shelf (NW climb) hold the west; the Breach is a narrow crossing band whose **waterline is now the organic contour of the drowned territory's edge**; the Scar claims the huge open surface north, south, and east; the Stage waits at one specific far point in the north-east, its lake and island intact. The campaign road **tours** the world — out of the town, north up the Shelf, east across the Breach, a long south-east swing through the Scar, then north to the Stage. Zone-seeded dressing replaces column-strip dressing; the painter keys the waterline/underwater grade off region weight fields; the runtime reads region per tile from the ground layer (grade tint, Nari's loss beat). BFS reachability green; 115 unit / sweep + overworld + obelisk + boss e2e green. |
| **12.2** | **2026-07-17** | **The seafloor, and the tracking mystery.** Owner: "where's the ocean floor environment?" — the Fold's ground is now true seafloor terrain: rippled silt-sand dunes with dark eelgrass beds, cross-faded into the Shelf (no more teal lawn). And: "along with naris footsteps there's tracks of other people, we have to understand and distinguish which ones are actually naris" — the ground becomes a **tracking puzzle** (§8.8.6): pilgrims' paired adult strides pace the road margins map-wide, three-toed den creatures cross the Scar, and single-file decoy small prints wander the regions beyond the Breach. **Nari's signature is learnable**, taught diegetically while he walks behind Mir in the Fold: paired gait, heel dot, faint right-foot drag. Decoys carry none of it. All ground marks, no overlays (v12.1 discipline). |
...
| **11.2** | **2026-07-16** | **The purge — the ground is the game.** Owner: "throw out anything that generated art that isn't the grass/water/ground/path/boss ground areas" (confirmed literal: cast included). Deleted: all 185 env kit pieces, NPCs, the title key-art, and the HD Mir/foe sheets. The painted ground plate (grass, water, rock mesas, path ribbons, baked fight clearings incl. the boss ring) is now the only shipped generated-era art. Every figure renders as a deliberately minimal code-drawn glow-shape (`tools/pixelart/placeholder_cast.py`: dark silhouette + one accent matching its runtime aura, same sheet contracts so zero engine churn); the save obelisk gets a minimal slab. Menus fall back to the dark vignette. All draw sites already guarded on `textures.exists`, so the dressing/venue/gate/canopy code paths stay for future art. `hd_cast.py` remains the regeneration path. 115 unit / e2e sweep + obelisk/overworld specs green; screenshot-reviewed. |
| **11.1** | **2026-07-16** | **The vivid pass.** Owner: "you're generating shit way too dark. it's gotta be like hyperlite drifter." All hd_cast generations re-cut to a vivid saturated HLD register; title key-art regenerated to match. *(Superseded hours later by the v11.2 purge — process rules live on in `hd_cast.py`.)* |
...
| **9.0** | **2026-07-14** | **Design audit 2 — the ground is painted, not stamped.** Owner: "the art is so terrible and not cohesive… HLD detail on rocks but chunky blocks everywhere." Root-caused in [`docs/design/design-audit-2.md`](../design/design-audit-2.md): the world mixed two registers — painterly AI sprites over a hand-stamped 16px tile carpet (repeated rock-blob tiles beside painted sea stacks, copy-paste road strips, panel-edged water, wallpaper grass). Fix: **tile stamping is retired**. New `tools/overworld/paint_ground.py` bakes the whole ground as ONE painted plate at 2× texel density (deterministic): rock clusters become single **mesa landforms** (organic craggy silhouettes, cracked lit tops, striated south cliff faces, cast shadows — the HLD cliff read); roads become continuous worn **ribbons** (distance-field edges, centerline wear, scattered stones); water becomes unified **bodies** (shore-distance depth ramp to near-black, jittered organic shorelines, foam + bank, sparse hashed swells); grass becomes a multi-scale noise **field** with per-pixel grain, pooled darks, and hash-placed tufts — plus quantized value ramps for pixel-art crunch and region bases blended over a wide band (bakes the seam cross-fade). The tile layer stays authoritative for collision/terrain but no longer renders; the v8.8 runtime shore/terrace passes are deleted as baked; props gained placement jitter (G5). Screenshot-verified across regions: no repeated stamp identifiable anywhere. 115 unit / 17 e2e green. |
| **9.14** | **2026-07-16** | **Fights scar the ground (closing the combat-drama gap).** A direct side-by-side against real HLD screenshots (built and reviewed critically, not just asserted) found our exploration screens hold up but our COMBAT moments read calmer than HLD's boss-room drama — no persistent mark that a fight happened here, unlike HLD's lingering blood/mud decals. `WorldFight` now stamps 2-3 small scuffs into a per-fight `RenderTexture` (depth 3, above the plate/dressing, below every fighter) on every landed hit — dark red at the player's feet, a darkened (0.6×) version of the foe's own accent colour at the foe's, so it reads as a bruise/ichor stain in that creature's colour family rather than a bright paint splash. Persists for the fight's duration, cleared when the scene tears down. Skipped under photosensitivity-safe mode. Tuned through direct visual iteration: too small/pale to see at first pass, recalibrated (alpha, scale, saturation) until confirmed visible in a cropped in-game capture without reading as gaudy. A hypothesis raised in the same comparison — that our fixed 2-colour (teal/amber) emissive glow palette was flattening scene colour variety — was tested against extracted hot-pixel data from all 23 glowing pieces and found NOT to hold up (the art is already intentionally warm-toned per region by design); no change made there, since a real problem wasn't actually present. 115 unit / 17 e2e green. |
| **9.13** | **2026-07-16** | **Every AI-generated sprite gets a forced dark outline.** `tools/pixelart/skatopia.py`'s own `outline()` docstring calls a 1px near-black silhouette ring "the single biggest 'reads as real pixel art' win" — every hand-authored piece (the colossal Conductor, `ow_props`, `ow_npcs`, `ow_landmarks`) has always had it; the AI-generated pipeline (env/band/enemies) never did, leaving those silhouettes to blend softly into the ground even after flat-cel quantization. New `outline_pass.py`, two techniques by asset shape: standalone env pieces get a canvas-grown ring (like `skatopia.outline()`, vectorized) — safe because `WorldScale.ts` reads render scale from the live texture height, so a 2px-taller PNG self-corrects with zero config changes; multi-frame band/enemy SHEETS get an in-place ring recolor instead, since BootScene hardcodes frame dimensions and growing the canvas would desync every frame boundary. Both idempotent (verified: a second run touches zero files). Every character, foe, and prop now has the crisp dark rim that separates it from the painted ground at a glance. 115 unit / 17 e2e green. |
| **9.12** | **2026-07-15** | **The cast joins the register; retired-era art deleted.** Owner: "redo all the art to the same quality." An asset-by-asset audit found the fidelity work had stopped at the environment: band and foe sheets were despilled/hardened (v9.11) but never palette-quantized, so characters still carried soft AI gradient shading next to the now-flat environment. `requantize_cast.py` computes a colour budget from area-PER-FRAME (not the whole multi-frame sheet, which had been inflating the budget) and requantizes band (13–14 colours) and foes (12–14) to genuine flat cel blocks, preserving emissive highlights (eyes, embers). The audit also found six categories of ALREADY hand-authored, already-flat pixel art needing no rework (`ow_props`, `ow_npcs`, `ow_landmarks`, the colossal Conductor, `battle_abyss`/`bg_title` — all procedurally painted via the palette-key `skatopia.py` pipeline from the start) — confirmed by direct visual inspection rather than assumed. Separately: a full trace of every BootScene asset load against actual draw call sites found **five retired-era categories loaded but never rendered by any live scene** — `hero_${role}` battle sheets and `assets/sprites/heroes/` (pre-band placeholder party, four unused strips + an orphaned "Amir" folder), five `arena_*.png` background images (superseded by the in-world venue system, PRD v9.4), `battle_conductor.png`/`caustics.png` (dead alongside them), and `ui_icons.png` (superseded by the monospace TextMenu UI). Deleted outright per the project's standing discipline (v8.3: "deleted outright, git history is the archive") rather than redone — dead art can't be brought to a quality bar it will never be seen at. 115 unit / 17 e2e green. |
| **9.11** | **2026-07-15** | **One pixel register.** Owner: \"still pixelly as fuck and when I look at HLD it's not.\" Root cause found: not the art content but MIXED PIXEL DENSITY — pieces rendered at fractional scales (0.28–1.6), so every sprite had different-sized, non-integer texels: gritty shimmer instead of HLD's uniform chunky pixels. Fix: `bake_world_scale.py` resamples every environment piece to its canonical world size (same metre table as WorldScale.ts) so everything renders at ~1.0 — one texel = one world pixel, everywhere — then `fidelity_pass.py` re-registers at final resolution; the runtime snaps residual scales to halves so texels stay integer-sized on the 2× canvas. Landform/gate/obelisk scales normalized to 1.0. Also: **per-region ambient grade** (a low-alpha additive wash of the local accent, tint follows the camera across regions — each region owns its light), **venue stage lights** (fights read as lit stages from across the map), and 3 ambiguous-art pieces rerolled (the strength-test machine no longer reads as a tub). Screenshot-verified: the saltmines workyard now reads as one uniform chunky register end to end. 115 unit / 17 e2e green. |
| **9.10** | **2026-07-15** | **One world unit + the world moves.** Owner: \"the scale of everything is totally wrong... why is a chair bigger than a building.\" Root cause: piece pixel-sizes were assigned ad hoc per generation batch — relative scale between pieces was noise. New `WorldScale.ts`: a canonical table of real-world heights in METRES anchored to the player (25px ≈ 1.7m, ~15px/m; small items get a readability boost with a 9px floor; unknown keys default small, never huge). The runtime computes every piece's render scale from its texture height at draw time — dressing, venues, and gates all use it, so art regeneration can never break world scale again. Buildings went up (ticket booth ~3.2m), furniture came down (chair 1m, goblet 0.22m). **And the world MOVES** (the HLD alive-air axis): 2-3 fireflies orbit every flame, teal glints pulse on open water, every canopy crown sways — all deterministic, all skipped under reduced motion. 115 unit / 17 e2e green. |
| **9.9** | **2026-07-15** | **The hand-pixeled fidelity pass (the last HLD axis) + green-screen despill.** AI pieces read soft next to HLD's hand-placed pixels: anti-aliased gradients, hundreds of near-duplicate colours, and background bleed locked into silhouette edges (owner: \"you need to do a green screen style removal for pixels on these objects\"). `tools/pixelart/fidelity_pass.py` rewrites all 173 environment pieces into the flat-cel register: median-cut palette quantization scaled by sprite area (~8 colours for a 14px prop, ~24 for a colossus), hard 0/255 alpha, isolated-speck removal that PRESERVES 1px features (a cello's neck survives; the first cut severed it), edge decontamination that recolours only genuinely contaminated ring pixels (neutral-grey + far from interior — deliberate outlines untouched), emissive-core preservation (median cut had snuffed the lanterns), and the same /9 value ramp the painted plate uses — pieces and ground share one colour register. Idempotent; runs after any generation batch. Before/after verified at 6× on green. |
| **9.7.1** | **2026-07-15** | **Every prop is placed with intention.** Owner: \"why is there a random bottle scattered for no reason. items need to be placed with intention. none of this scattering shit... why do I see 70 bottles in one scene.\" Root cause: the hash-scatter stamped the same few kit pieces across hundreds of cluster tiles while most of the library never appeared. The scatter system is **deleted** (hash loop + hash landform placement). Replacement: **authored set-dressing** (`src/data/content/overworld/dressing.json`) — 288 placements composing **128 story vignettes** (two authored waves) (a tide-watcher's cold camp, the wreck of the Pale Sister, a figurehead shrine, a foreman's abandoned shift…), singletons, and landmark landforms, authored by a 10-agent workflow (one scene-dresser + one adversarial placement-verifier per region) against the region terrain maps, then hard-validated by `tools/overworld/merge_dressing.py` (anchor-on-grass, venue/echo clearance, per-key budgets: scatter ≤2, filler ≤4, landform ≤1 per region). Each region reads sparse and deliberate; no piece repeats more than twice per region. 115 unit / 17 e2e green. |
| **9.7** | **2026-07-15** | **The HLD grade — the world is finally in COLOUR.** Owner set the bar: \"look up a picture of hyperlite drifter and if the comparison isnt that ours is better then you keep going.\" Side-by-side against HLD's Steam screenshots, ours read uniformly muddy-olive: a 0.18 cold-overcast wash, a vignette reaching 75px into a 180px screen, a 0.16 dark dapple, a chroma-crushing scenery tint, and grey-ish plate palettes were stacked on every frame. All five muting layers regraded: region palettes pushed to one hot signature hue each (teal / amber / orchid / rust-orange / violet, grass base brightened, accent mix 0.22→0.36), the road ribbon repainted warm pale bone (playfield-vs-surround value pop), water deeps gone blue-violet, the map's outer edge sunk into a near-black organic frame (HLD's lit-place-inside-a-void value structure), overcast 0.18→0.06, vignette pulled to a rim, scenery tint made chroma-preserving, god rays strengthened, and a dozen ambient light motes drift the air. Screenshot-verified across regions — each now owns a colour identity with complementary value drama. 115 unit / 17 e2e green, zero flakes. |
| **9.6** | **2026-07-15** | **Curved trails + region-entry gates.** **(1)** The meandering road's single-tile jogs painted as hard stair-steps; the path mask's falloff widened (blur 7→11) so every corner melts into a curve — the trail now reads hand-worn end to end. **(2)** Where the main road crosses each region boundary, the NEXT region announces itself: its most iconic landform (mine headframe, collapsed big-top, grandfather clock, organ facade) stands north of the road and one of its lit props (lantern/torch/lamp/candelabra, with a warm light pool) flanks the south side — every border crossing is a composed gateway instead of a tint change. Screenshot-verified at all four gates; the hall entry (causeway over dark water, torch + candelabra framing the organ facade) is the mood target realized. 115 unit / 17 e2e green. |
| **9.5** | **2026-07-15** | **The map itself is de-gridded (scale/placement audit SP9 + SP11 closed).** The last two "artificial" reads came from map DATA, not paint: `_carve_path` corridors ran on rails (dead-straight parallel N-S roads in the saltmines) and the attic's four partitions were identical 1-wide full-height walls at exact 5-column spacing. `generate_overworld_map.py` now carves a **meandering main road** (sideways drift every few tiles, endpoints exact, still BFS-validated for full reachability), **segments the saltmines ridges** into worked terraces with jittered rows and gaps, and **varies the attic partitions** (jittered x, staggered ends, widths 1–2, alley breaks). Secret-spur corridors stay plain-L on purpose — their pocket punch-through must stay predictable. Plate repainted; screenshot-verified (mine reads as terraced workings, attic as an irregular maze, the road as a worn trail). 115 unit / **17 e2e, zero flakes this run**. |
| **9.4** | **2026-07-15** | **Venues ARE the world.** Owner: "you just overlayed the boss arenas on-top of normal places and you can see through the boss arenas... straight up squares destroy the aesthetic" + "all of these images have floors on them that don't match the flooring... cement under everything, then it goes to grass." **(1)** The runtime venue floor (a translucent 320×180 rect over the map) is deleted — `paintFloor`/`composeArena` removed as retired code. Instead every fight node gets an organic **trampled-earth clearing baked into the painted plate** (`paint_ground.py`: ragged noise edge, wear rings, region-tinted packed earth; the boss gets a grand 2× ring that bridges the hall lake as one deliberate stage). WorldFight's always-walkable circle now stands on ground that IS the world. **(2)** Every generator camera demands "NO ground under it — the object ends at its own feet": all 15 venue pieces regenerated clean, canopy trees regenerated + their residual soil discs converted to shadow tone (universal on any floor). **(3)** Emissive scatter (lanterns, crystals, braziers, candles, tide pools…) casts additive light pools — the night world reads lit, not decorated. **(4)** Causeway pass widened so path-through-lake crossings re-tone fully; mesa plateaus dressed (SP10: tonal patches + lit-edge pebbles). **(5)** Scatter wave 3 ships: 24 pieces per biome (120 total), 9 rerolled on review. Screenshot-verified at every node; the slime clearing and attic ring read composed and intentional. 115 unit / 17 e2e green. |
| **9.3** | **2026-07-15** | **Scale & placement audit — the entire landscape.** Owner: "audit of all the art in the game where you just focus on scale and placement." Systematic 5×3 camera sweep of every region at three latitudes plus fight/venue captures; 11 findings logged in [`docs/design/scale-placement-audit.md`](../design/scale-placement-audit.md), 8 fixed same-day: paths routed through lakes now paint as built stone **causeways** (dark-edged masonry, not glowing squiggles on water); venue set pieces take a terrain predicate so nothing **floats on water**; colossal outcrops need their whole footprint clear so none **squat on roads**; in-cluster scatter goes checkerboard so 20-30px pieces on a 16px grid stop **piling into heaps**; the attic doll/rocking-horse shrunk from hero-size to **toy scale**; ponds gained a coarse boundary **warp** (no more rounded rectangles); scatter clears node venues and echo spots; townsfolk normalized to just under hero height. 3 low-severity items deferred with owners (attic pillar repetition, bare mesa tops, straight parallel roads — all map-data work). Re-swept after fixes; all captures clean. 115 unit / 17 e2e green. |
| **9.2** | **2026-07-15** | **Design audit 3 stages D–E — the bestiary reads apart; the venues are objects, not doll-houses.** **(D)** The three pipeline foes had homogenized into near-identical hooded ghosts under a shared palette clause — the same failure the band had. Each foe regenerated with a distinct silhouette + one signature colour (`newfoes.py`): the slime is an actual heaped TOXIC-TEAL ooze blob, the drifter a RUST-ORANGE coated wanderer, the elite wraith a VIOLET-robed bone-crowned spectre; the hand-authored colossal Conductor untouched. **(E)** Contact-sheet audit of the original envkit found 15 of 28 venue pieces were room-corner DIORAMAS (a whole miniature room where a dresser should be) or flood-key shreds — the framing failure the scatter pipeline already solved. All 15 rerolled as isolated objects with contrast-aware backgrounds (`venue_reroll.py`), keeping each piece's logical size so every `ArenaLayout` placement keeps working. 115 unit / 17 e2e green. |
| **9.1** | **2026-07-15** | **Design audit 3 stages A–B — retina render, a band you can tell apart, and the lamp is gone.** Owner: "things are still WAY too pixelly. the band members are all indistinguishable, theres a blurry ass lamp scattered literally everywhere… I thought we needed to generate like 500 assets but its looks like you only generated 20?" **(A) Retina:** the canvas now renders at 2× (640×360) with every camera zoomed 2× over the unchanged 320×180 design space (`RENDER_SCALE`/`retinaCamera`), so dense-texel art (72px cast at ~0.5 scale, the 32px-per-tile ground plate) finally shows its real detail; screen-pinned UI re-anchored per frame (`pinToScreen`) since scrollFactor-0 drifts under scrolled zoomed cameras. **(B) Cast + scatter:** the four band members regenerated with one dominant signature colour each (Amir: bright red guitar + teal mohawk; bassist: the dark hooded one; vocalist: ember-orange greatcoat; drummer: plum + gold snare) — distinct at overworld scale, contact-sheet verified. The shared 6-frame prop sheet (the repeated lamp) is replaced by **per-biome scatter kits**: 8 object-framed AI pieces per region (`tools/pixelart/scatterkit.py`, 40 total) auto-loaded by the env glob; `OverworldScene` scatters only from the local biome's kit. Contact-sheet review rejected 18 first-batch pieces (dark-on-dark generations shredded by the flood key) — rerolled with contrast-aware backgrounds (`scatter_reroll.py`); cluster coverage raised so the kits actually show. **(C) Volume:** scatter wave 2 doubles every kit to **16 pieces per biome** (80 total), and each biome gains **landform variants** — 3 outcrop + 2 canopy silhouettes (shipwreck, lighthouse stump, mine headframe, salt-crystal bloom, collapsed big-top, ferris wheel, grandfather clock, book tower, cloak tree, organ facade, candelabra tree…) picked deterministically per cell so no region repeats one form wall-to-wall. ~26 generations failed review across rounds (room interiors, framed cards, island-filter shreds, washed-out hall marble) and were re-prompted per the review-and-reroll discipline. In-game verified across regions. 115 unit / 17 e2e green. |
...
strides along the road margins), den creatures (three-toed crossings on the Scar),
and — cruelest — **other small prints** that are not Nari's, wandering single-file
beyond the Breach toward dens and dead ends. **Nari's signature is learnable**: while
he follows Mir through the Fold, the player watches his real prints — paired gait, a
heel dot, and a faint right-foot drag. Decoy prints have no heel and no drag.
Distinguishing them is the intended difficulty of the search; no UI ever labels a
track. *(Future hook, v2: an interactive "examine track" verb that lets Mir kneel and
confirm a print — deliberately absent in v1 so the read stays a player skill.)*

**8.8.5 Save-obelisks — the cult's prayer stones (v12.0).** The Fold prays to the
obelisk, and its faith marks the whole ascent. At the heart of the town stands the
**massive obelisk itself (v14.0)** — a tall monolith on a painted stone dais, ringed
by kneeling worshippers, the thing the two Fold echoes speak of; it is the town's save
point and no fight ever happens in its sanctuary (the campaign begins only once Mir
climbs out of the Fold). Its smaller kin, a save-obelisk, stands near each fight
...
| Title key-art + framed UI kit (plates, boss bar) | `assets/ui/`, `tools/pixelart/ui.py` | UI kit shipped; title key-art **purged v11.2** (menus use the dark vignette; slot open) |
...
| Cast | Mir strips (HD base pose → derived strips) | **purged v11.2** → minimal placeholder shapes; regeneration path: `hd_cast.py` |
| Foes | slime/drifter/wraith idles + both Conductor sheets | **purged v11.2** → minimal placeholder shapes |
| **Golden scene** (§11.1.2) | fight capture, frozen as reference | **reset** — the ground-only world is the new baseline; freeze after the next character direction is approved |
| Environment kits | 5 biomes (landforms → venue kit → scatter) | **purged v11.2**; slots open, batch only after the golden scene freezes |
| Overworld figures | NPCs (purged), props sheet, echo rune, save obelisk (placeholder slab) | open |
