...
**"Enterprise level" acceptance criteria (v6.4).** Art quality is judged against these checkable standards, not vibes: (1) **no tint-swaps or upscales** — every character is bespoke authored art, and bosses are authored at native colossal resolution (never a small sprite scaled up); (2) **animation states, not static sprites** — the playable character has authored attack poses (anticipation → impact) and a hurt reaction; enemies have idle motion, a readable windup telegraph, and a hurt flash; (3) **every arena is a distinct authored place** meeting §11.1.1; (4) **a consistent light source and emissive pass** on every sprite (rim light, outline, glow); (5) **one master palette** across all assets; (6) all of it deterministic and regenerable (`generate_all.py`). Gaps against these criteria are tracked in §20.4 — anything not yet meeting them must be listed there, not implied shipped.
...
**Overworld:** the same rule applies at lower intensity — the hub map must read as the *surface road between these places* (calcified milestones near the mine road, lantern strings near the pit descent, etc.), staged with the existing prop system so approaching a marker foreshadows its arena.
