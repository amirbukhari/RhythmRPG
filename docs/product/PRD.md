...
- **v7.0 exploration — in progress:** the five-region joined world map, echoes (§8.8.2), and hidden paths/secrets (§8.8.3) are specified but not yet built; see the task list for this cycle. Until landed, the game still ships the v5.0-shape single-road overworld, which is now explicitly a placeholder for §8.8, not the target.
